...
const AssetOverview: React.FC<AssetOverviewProps> = ({
  ...
  const { total, distribution } = useMemo(() => {
    ...
    // 在 loading 状态下，为了保持列表稳定，我们构造一个假的分布列表
    if (isLoading) {
      ...
      return { total: 0, distribution: distItems };
    }
    ...
    >
      ...
      <div className="relative z-10 w-full mb-8">
        ...
        <div className="flex items-baseline gap-1">
          ...
          {isLoading ? (
            <SkeletonPulse className="w-48 h-10 ml-2" />
          ) : (
      ...
      <div className="flex items-center justify-between gap-8">
        <div className="relative size-32 flex items-center justify-center shrink-0">
          ...
          >
            ...
            {isLoading ? (
              ...
              distribution.map((item) => (
                ...
                >
                  <title>
                    {item.label}: {formatNumber(item.percentage, 1)}%
                  </title>
                </circle>
              ))
            )}
          ...
          {/* Total 100% 始终显示，即使在 loading 状态下 */}
          ...
          {distribution.map((item) => (
            ...
            >
              <div className="flex items-center gap-2">
                <div
                  className={`size-2 rounded-full ${item.bgColor || item.color.replace("text-", "bg-")}`}
                ></div>
                <span className="text-xs font-medium text-slate-300 group-hover:text-white transition-colors">
                  {item.label}
                </span>
              </div>
              <span className="text-xs font-bold text-white flex items-center justify-end min-w-[40px]">
                {isLoading ? (
                  <>
                    <SkeletonPulse className="w-6 h-3 mr-0.5" />
